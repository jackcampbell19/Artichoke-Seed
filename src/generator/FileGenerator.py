...
class FileGenerator:
    ...
    def __init__(self, max_continuous_length: float):
        self._max_continuous_length: float = max_continuous_length
    ...
    def convert_outline_to_instructions_v1(self, outline: List[Tuple[int, int]]):
        fx, fy = outline[0]
        self._instructions.append(
            ig.move(fx, fy, self._clearance_z, subspace=True)
        )
